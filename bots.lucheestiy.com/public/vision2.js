...
const I18N = {
  ...
  ru: {
    ...
    notify_denied: "Уведомления запрещены браузером",
    fleet_online: "Онлайн",
    fleet_warning: "Внимание",
...
function initDetailsUi() {
  ...
  $("chartWindow").addEventListener("change", () => {
    ...
    if (bot) renderUsageChartsExtended(bot);
  });
...
function renderFleetHealth(data) {
  const section = $("fleetHealth");
  const bar = $("fleetHealthBar");
  const legend = $("fleetHealthLegend");
  if (!section || !bar || !legend) return;

  const bots = Array.isArray(data.bots) ? data.bots : [];
  if (!bots.length) { section.hidden = true; return; }

  let online = 0, warning = 0, offline = 0;
  for (const bot of bots) {
    const status = getStatusInfo(bot);
    if (status.class === "online") online++;
  ...
  const segments = [
    { cls: "online", count: online, label: t("status_online") },
    { cls: "warning", count: warning, label: t("status_warning") },
    { cls: "offline", count: offline, label: t("status_offline") },
  ];
  ...
  for (const seg of segments) {
    ...
    el.addEventListener("click", () => {
      ...
      const filterMap = { online: "active", warning: "issues", offline: "issues" };
      const target = filterMap[seg.cls] || "all";
      showSel.value = showSel.value === target ? "all" : target;
      state.ui.show = showSel.value;
  ...
  legend.innerHTML = segments
    .map(s => `<span class="legend-item" title="${escapeHtml(s.label)}: ${s.count}">
      <span class="legend-dot ${s.cls}"></span>${s.count} ${escapeHtml(s.label)}
    </span>`)
    .join("");
}
...
// Summary Sparklines
// ============================================
function drawMiniSparkline(container, values, color) {
  if (!container) return;
  container.innerHTML = "";
  ...
  const w = rect.width;
  const h = rect.height;
  const vals = Array.isArray(values) && values.length > 1 ? values : [];
...
function renderSummarySparklines(data) {
  const bots = Array.isArray(data.bots) ? data.bots : [];

  // Aggregate daily data across all bots
  const dailyAgg = {};
  for (const bot of bots) {
    const daily = bot.usage && bot.usage.daily30d ? bot.usage.daily30d : [];
    for (const d of daily) {
      if (!d.date) continue;
      if (!dailyAgg[d.date]) dailyAgg[d.date] = { tokens: 0, cost: 0, errors: 0 };
      dailyAgg[d.date].tokens += d.tokens || 0;
      dailyAgg[d.date].cost += d.costUSD || 0;
  ...
  const tokensData = last7.map(d => dailyAgg[d].tokens);
  const costData = last7.map(d => dailyAgg[d].cost);
  const errorsData = last7.map(d => dailyAgg[d].errors);

  // Active bots per day (approximate: use current count as flat line sparkline isn't useful)
  // Instead, draw a flat-ish line based on total
  drawMiniSparkline($("sparklineBots"), tokensData.map(() => bots.filter(b => b.systemd && b.systemd.activeState === "active").length), "#a78bfa");
  drawMiniSparkline($("sparklineTokens"), tokensData, "#22d3ee");
  drawMiniSparkline($("sparklineCost"), costData, "#60a5fa");
  drawMiniSparkline($("sparklineErrors"), errorsData, "#fb7185");
}
...
function renderCostTrend(data) {
  ...
  const today = dailyAgg[dates[dates.length - 1]] || 0;
  const yesterday = dailyAgg[dates[dates.length - 2]] || 0;

  const costEl = $("statCost");
  if (!costEl) return;

  // Remove existing trend badge
  const existing = costEl.querySelector(".trend-badge");
  ...
  let pct, cls, arrow;
  if (yesterday === 0) {
    pct = 100; cls = "up"; arrow = "↑";
  } else {
    pct = ((today - yesterday) / yesterday * 100);
    if (Math.abs(pct) < 1) { cls = "flat"; arrow = "→"; }
    else if (pct > 0) { cls = "up"; arrow = "↑"; }
    else { cls = "down"; arrow = "↓"; }
  }
  ...
  badge.title = `vs yesterday: $${yesterday.toFixed(2)} → $${today.toFixed(2)}`;
...
const notifyState = {
  enabled: false,
  previousStatuses: new Map(), // unit -> status class
};

function initNotifications() {
  notifyState.enabled = lsGet("notify", "0") === "1";
  updateNotifyButton();
    ...
    btn.addEventListener("click", async () => {
      if (!notifyState.enabled) {
        if ("Notification" in window && Notification.permission !== "granted") {
          ...
          if (perm !== "granted") {
            showToast(t("notify_denied") || "Notification permission denied", "error");
            return;
          }
        }
      ...
      lsSet("notify", notifyState.enabled ? "1" : "0");
      updateNotifyButton();
      showToast(
        notifyState.enabled
          ? (t("notify_on") || "Notifications enabled")
          : (t("notify_off") || "Notifications disabled"),
        "info"
      );
    });
  }
...
function updateNotifyButton() {
  const btn = $("notifyBtn");
  if (btn) btn.classList.toggle("active", notifyState.enabled);
}

function checkBotStatusChanges(data) {
  ...
  for (const bot of bots) {
    const status = getStatusInfo(bot);
    newStatuses.set(bot.unit, status.class);

    const prev = notifyState.previousStatuses.get(bot.unit);
    if (prev && prev !== status.class) {
      // Status changed
      if (status.class === "offline" && prev === "online") {
        new Notification("Bot Offline", {
          body: `${bot.displayName || bot.unit} went offline`,
          icon: "/favicon-192x192.png",
          tag: `bot-offline-${bot.unit}`,
        });
      } else if (status.class === "online" && prev === "offline") {
        new Notification("Bot Online", {
          body: `${bot.displayName || bot.unit} is back online`,
          icon: "/favicon-192x192.png",
          tag: `bot-online-${bot.unit}`,
        });
      } else if (status.class === "warning") {
        new Notification("Bot Warning", {
          body: `${bot.displayName || bot.unit} has issues`,
          icon: "/favicon-192x192.png",
          tag: `bot-warn-${bot.unit}`,
        });
      }
    }
  }

  notifyState.previousStatuses = newStatuses;
}
...
function downloadLogs() {
  const raw = state.details.logs.raw;
  const unit = state.details.logs.unit || "logs";
  if (!raw) {
...
function renderUsageChartsExtended(bot) {
  ...
  const dailyAll = bot.usage && bot.usage.daily30d ? bot.usage.daily30d : [];
  const daily = mode === "7d" ? dailyAll.slice(-7) : dailyAll;
  const errors = daily.length ? daily.map((d) => d.errors || 0) : [0];

  drawBars($("errorsChart"), errors, "#fb7185");
}
...
async function refresh() {
  ...
  try {
    ...
    renderSummarySparklines(state.data);
    renderCostTrend(state.data);
    checkBotStatusChanges(state.data);
